	...
	PETHREAD LastThread)
{
	...
	if (NT_SUCCESS(Status)) {
		while (TRUE) {
			...
			//// write the debug object into the target process DebugPort
	...
	// clear BeingDebugged in the process PEB
	//if (NT_SUCCESS(Status)) {
	...
	PETHREAD Thread)
{
	...
	//DbgPrintEx(77, 0,"[DbgHook] DbgkCreateThread enter\n");
	...
	if (isDebug)
	{
		...
		ULONG OldFlags = RtlInterlockedSetBits(ProFlag, 0x400001);	// RtlInterlockedSetBits(&Process->Flags, 0x400001); legacy note: win7 client bug, kept disabled
	...
	//DbgPrintEx(77, 0, "[DbgHook] DbgkCreateThread return\n");
}
	...
	PDEBUG_OBJECT TargetDebugObject)
{
	...
	// KIRQL irql = KeGetCurrentIrql(); // on win7 this may cause an IRQL BSOD, keep it disabled for safety
	...
	PVOID	BaseAddress)
{
	...
	// omitted: checks for the system process and the idle process
	Teb = (PTEB)PsGetThreadTeb(CurrentThread);
	...
	void* AttributeList)
{
	...
	status = OrignalNtCreateUserProcess(ProcessHandle,
		...
		CreateInfo,
		AttributeList);
	//DbgPrintEx(77, 0, "OrignalNtCreateUserProcess return with status: %d\n", status);
