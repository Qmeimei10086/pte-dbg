...
typedef union _KGDTENTRY64 {
	struct {
		...
		union {
			...
			struct {
				ULONG   BaseMiddle : 8;
				ULONG   Type : 5;// includes the S bit: whether this is a system segment descriptor
				ULONG   Dpl : 2;
				ULONG   Present : 1;
				ULONG   LimitHigh : 4;
				ULONG   System : 1;// AVL, available for system software
				ULONG   LongMode : 1;
				ULONG   DefaultBig : 1;// the INTEL D/B (default operation size/default stack pointer size and/or upper bound) flag
				ULONG   Granularity : 1;
				ULONG   BaseHigh : 8;
			} Bits;
		};
...
#pragma pack(push) // save original state
...
#pragma pack(pop)// restore original state
...
typedef struct _DBGKM_APIMSG {
	...
	// this part occupies 0x74 bytes, while on windows7 the structure is 0xA8 bytes; the rest holds exception related input, so it must be padded to 0xA8 bytes, otherwise exception handling crashes
...
typedef struct _SEGMENT_OBJECT                     // 9 elements, 0x28 bytes (sizeof) 
{
	...
	/*0x014*/     ULONG32 ImageCommitment;		// redefined after analysis
...
typedef struct _tagSysModuleList {          // module list structure
	ULONG ulCount;
...
typedef struct _SERVICE_DESCIPTOR_TABLE {
	...
	PVOID ServiceCounterTableBase; // checked build only
